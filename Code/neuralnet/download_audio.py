...
def format_audio(input_audio_file,output_audio_file):
    ...
    cmdstring = "ffmpeg -loglevel panic -i %s -ac 1 -ar 16000 %s" %(input_audio_file,temp_audio_file)
    os.system(cmdstring)
...
def download_audio_method(line,csv_file):
    ...
    tag = "_".join("@".join(line.split(",")[3:]).strip().strip('"').split("/"))
    ...
    try:
        ...
        path_to_download = output_folder + "/Y" + query_id + "@" + tag + "." + bestaudio.extension
        ...
        path_to_formatted_audio = formatted_folder + "/Y" + query_id + "@" + tag + ".wav"
        ...
        path_to_segmented_audio = segmented_folder + "/Y" + query_id + '_' + start_seconds.strip() + '_' + end_seconds.strip() + "@" + tag +  ".wav"
    ...
    with open(csv_file, "r") as segments_info_file:
        with open(error_log, "a") as fo:
            for line in tqdm(segments_info_file):
                line = (line,csv_file)
                download_audio_method(line, csv_file)
